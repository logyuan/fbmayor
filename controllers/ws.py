...
def timeseries():
    ...
    for row in rows:
        date.append(row.date_time.strftime('%Y/%m/%d'))
    ...
    for row in rows:
        ...
        date = row.date_time
        #rows1 = fbpl((fbpl.team_counts.team == "連勝文") & (fbpl.team_counts.date_time==date)).select(fbpl.team_counts.ALL, orderby=fbpl.team_counts.date_time).first()

        likes.append(row.likes)
        totalPosts.append(row.total_active_posts)
        totalPostLikes.append(row.total_post_likes)
        totalPostComments.append(row.total_post_comments)
        totalPostShares.append(row.total_post_shares)

        row2 = fbpl((fbpl.team_counts.team == "柯文哲") & (fbpl.team_counts.date_time==date) ).select(fbpl.team_counts.ALL, orderby=fbpl.team_counts.date_time).first()

        likes.append(row2.likes)
        totalPosts.append(row2.total_active_posts)
        totalPostLikes.append(row2.total_post_likes)
        totalPostComments.append(row2.total_post_comments)
        totalPostShares.append(row2.total_post_shares)

        r1.append(likes)
        r2.append(totalPosts)
        r3.append(totalPostLikes)
        r4.append(totalPostComments)
        r5.append(totalPostShares)

    result.update({"likes":r1})
    result.update({"totalPosts":r2})
    result.update({"totalPostLikes":r3})
    result.update({"totalPostComments":r4})
    result.update({"totalPostShares":r5})
...
def exportElectionPath():
    ...
    for candidate in candidates:
        ...
        for page in Pages:
            ...
            likes_sincelastupdate += page.likes_sincelastupdate
            talking_about_sincelastupdate += page.talking_about_sincelastupdate
        picture = page.picture
        url_facebook = page.link
        ...
        ts.update({"image": picture})
        updateTime = str(page.updated_time)
        ...
        ts.update({"url_facebook": url_facebook})
        result.update(ts)
...
#--------------------------------------------------------------------------------------------------------------#
        ...
        for Post in Posts:
            ...
            ts.update({"shares_sincelastupdate": Post['shares_sincelastupdate']})
            ts.update({"comment_sincelastupdate": Post['comment_sincelastupdate']})
            ts.update({"likes_sincelastupdate": Post['likes_sincelastupdate']})
...
#--------------------------------------------------------------------------------------------------------------#
        ...
        for story in Stories:
            ...
            ts.update({"shares_sincelastupdate": story['shares_sincelastupdate']})
            ts.update({"comment_sincelastupdate": story['comment_sincelastupdate']})
            ts.update({"likes_sincelastupdate": story['likes_sincelastupdate']})
...
#--------------------------------------------------------------------------------------------------------------#
        ...
        for link in Links:
            ...
            ts.update({"shares_sincelastupdate": link['shares_sincelastupdate']})
            ts.update({"comment_sincelastupdate": link['comment_sincelastupdate']})
            ts.update({"likes_sincelastupdate": link['likes_sincelastupdate']})
...
#--------------------------------------------------------------------------------------------------------------#
        ...
        for video in Videos:
            ...
            ts.update({"shares_sincelastupdate": video['shares_sincelastupdate']})
            ts.update({"comment_sincelastupdate": video['comment_sincelastupdate']})
            ts.update({"likes_sincelastupdate": video['likes_sincelastupdate']})
